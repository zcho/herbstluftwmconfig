...
- **Блоки**: раскладка `ru`/`us` (клик → переключение), батарея (глиф FontAwesome,
  зависящий от уровня ``…``, при зарядке — ⚡; **клик → всплывающая подсказка**
  `panel_batip.py`: оценка остатка «до 0%» при разряде / «до полного» при зарядке +
  текущая мощность расхода/заряда в Вт; закрывается кликом), `Net` (SSID/сигнал,
