...
- **Угол-меню**: едва заметный глиф `⛧` (перевёрнутая пентаграмма, U+26E7) у правого
  края панели открывает по клику ЛКМ меню запуска на `rofi`. Список берётся из
  `apps.txt` — формат `Имя<TAB>команда`, строки с `#` игнорируются. Глиф рисуется
  шрифтом FreeMono через dzen `^fn()` (в UbuntuMono Nerd Font символа нет).
